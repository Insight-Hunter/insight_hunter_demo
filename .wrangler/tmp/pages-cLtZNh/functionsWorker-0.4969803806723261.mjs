...
var onRequestPost = /* @__PURE__ */ __name(async ({ request }) => {
  ...
  else if (s.includes("forecast")) a = "Base case shows steady growth next 3\u20134 months; stress test adds \xB120% demand shock and \xB115% CAC delta.";
...
var onRequestPost2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const { to, subject = "Insight Hunter Alert", text = "Demo alert fired." } = await request.json();
  const sender = env.SENDER_EMAIL || "no-reply@demo.insighthunter.app";
  const rcpt = to || env.NOTIFY_TO;
  if (!rcpt) {
...
var onRequestGet = /* @__PURE__ */ __name(async () => new Response("OK", { headers: { "content-type": "text/plain" } }), "onRequestGet");

// api/plan.ts
var onRequestPost3 = /* @__PURE__ */ __name(async ({ request }) => {
...
var onRequest = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (request.method === "POST") {
    ...
    const a = {
      ...
      email: body.email || env.NOTIFY_TO || "demo@example.com",
...
// api/profiles.ts
var onRequest2 = /* @__PURE__ */ __name(async () => {
  const items = [
    { id: "c1", kind: "client", name: "Acme Co", balance: 12500, aging: { current: 7e3, d30: 3e3, d60: 2e3, d90: 500 } },
    { id: "c2", kind: "client", name: "Globex", balance: 8200, aging: { current: 4e3, d30: 2200, d60: 1500, d90: 500 } },
...
// api/settings.ts
var memory = { emailDigest: true, currency: "USD", fyStart: 1, aiTone: "concise" };
...
var onRequest4 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  app.get(
    "/api/demo/forecast",
    (c) => c.json([
      { month: "Sep", cashIn: 28e3, cashOut: 21e3, netCash: 7e3, eomBalance: 42e3 },
...
// _middleware.ts
var onRequest5 = /* @__PURE__ */ __name(async ({ request, env, next }) => {
  const url = new URPaL(request.url);
  if (url.pathname.startsWith("/api/health") || url.pathname.startsWith("/assets") || url.pathname.startsWith("/data") || url.pathname === "/favicon.ico") return next();
  const user = env.DEMO_USER || "demo";
  const pass = env.DEMO_PASS || "demo";
...
// ../.wrangler/tmp/pages-cLtZNh/functionsRoutes-0.18334476309282555.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-CDIijX/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-CDIijX/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4969803806723261.mjs.map
